...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, inputs, onConfigChange, onTextInputChange, onGenerate }: CanvasConfigNodePanelProps) {
  ...
  return (
    <div className="flex h-full w-full flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }}>
      ...
      <div className="mb-2 flex min-w-0 items-center gap-2">
        ...
        {mode === "image" ? <CanvasSizePicker className="w-[92px] shrink-0 !h-10" value={node.metadata?.size || globalConfig.size || defaultConfig.size} onChange={(value) => onConfigChange(node.id, { size: value })} /> : null}
        <InputNumber min={1} max={15} className="canvas-control-number h-10 shrink-0 !w-[58px]" value={count} onChange={(value) => onConfigChange(node.id, { count: Number(value) || 1 })} />
      </div>
