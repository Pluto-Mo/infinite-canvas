    ...
    >
      ...
      <div className="mt-2 flex min-w-0 items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          {mode === "image" ? (
            <InputNumber min={1} max={15} className="canvas-control-number h-10 shrink-0 !w-[58px]" value={Math.floor(Math.abs(Number(config.count)) || 1)} onChange={(value) => onConfigChange(node.id, { count: Number(value) || 1 })} />
          ) : null}
        </div>
        <Button
          type="primary"
