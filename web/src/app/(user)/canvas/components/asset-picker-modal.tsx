...
function PickerCard({ title, kind, cover, loading, onClick }: { title: string; kind: string; cover: string; loading?: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      className="group relative cursor-pointer overflow-hidden rounded-lg border border-stone-200 bg-white text-left transition hover:border-blue-400 hover:shadow-md dark:border-stone-700 dark:bg-stone-900 dark:hover:border-blue-500"
      onClick={onClick}
      disabled={loading}
    >
      {cover ? (
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-blue-500/0 text-sm font-medium text-white opacity-0 transition group-hover:bg-blue-500/70 group-hover:opacity-100">
        插入画布
      </div>
    </button>
  );
